...
class AlliedUnit(Unit):           # definiujemy klasę sojuszniczej jednostki dziedziczącą po Unit
    ...
    def __init__(self, position, unit_type, target_group, bullet_group, sprite=None):
        ...
        # Jeśli sprite przekazany, użyj go, w przeciwnym razie stwórz domyślny prostokąt
        if sprite is not None:
            used_sprite = sprite
        else:
            used_sprite = pygame.Surface(stats["size"], pygame.SRCALPHA)
            pygame.draw.rect(used_sprite, stats["color"], used_sprite.get_rect())
